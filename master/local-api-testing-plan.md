...
```bash
# Validate env first
python3 scripts/local_api_harness.py validate-env

# Optional: load an additional profile
python3 scripts/local_api_harness.py --env-file .env.staging validate-env

# CRM read (provider is selected by CRM_PROVIDER)
python3 scripts/local_api_harness.py crm-read

# CRM upsert (requires LOCAL_API_ALLOW_WRITE=1)
python3 scripts/local_api_harness.py crm-upsert-company --domain example.com --name "DeployGTM API Harness"

# HubSpot aliases remain available
python3 scripts/local_api_harness.py hubspot-read
python3 scripts/local_api_harness.py hubspot-upsert-company --domain example.com --name "DeployGTM API Harness"

# one second API read
python3 scripts/local_api_harness.py one-second-read

# all three
python3 scripts/local_api_harness.py run-all --domain example.com --name "DeployGTM API Harness"
```
