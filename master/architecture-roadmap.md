...
2. **Research layer is always AI-first.** `engage.py` researches before asking. Context.md is populated by the system, not by forms Matthew fills in.

3. **Drive is supplementary, not required.** Every workflow degrades gracefully when Drive isn't configured.

4. **Confidence markers on all AI inferences.** context.md always labels `[confirmed]`, `[researched]`, `[inferred]`. Never present assumptions as facts.

5. **Scores are live, not frozen.** `score_engine.py` recomputes on every weekly refresh. Scores decay with signal age and rise with engagement events.

6. **Outreach gates on verified signals.** `verify_signals.py` blocks outreach on any account with VERIFY markers. This is not optional.

7. **No CRM writes without explicit confirmation.** `activate_account.py` requires `click.confirm()` for live pushes. Dry-run is always available.

8. **Every new client engagement starts with `make engage`.** Not `new-client`, not a blank template — the research-first intake is the standard.
