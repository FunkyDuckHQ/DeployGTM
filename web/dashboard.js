...
const nextAction = (route) => {
  const actions = {
    manual_sales_review_and_enrich: "Assign to founder/seller, enrich contacts, and draft founder-reviewed outreach.",
    enrich_and_campaign_test: "Enrich likely buyers and create a controlled message-market fit test.",
    enrich_selectively_or_monitor: "Monitor for urgency signals and enrich only if a timing event appears.",
...
const average = (items, key) => items.length ? items.reduce((sum, item) => sum + number(item[key]), 0) / items.length : 0;
...
function render() {
  const filtered = activeRoute ? allSnapshots.filter((item) => item.recommended_route === activeRoute) : allSnapshots;
...
async function loadManifest() {
  const response = await fetchFirst(["data/dashboard_manifest.json", "dashboard_manifest.json"]);
