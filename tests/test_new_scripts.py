...
class TestSignalsToMatrix(unittest.TestCase):
    def setUp(self):
        ...
        # Load the real script then patch its paths
        # Need generate_outreach available for import
        gen = _load(
            "generate_outreach",
            MATRIX_SCRIPTS_DIR / "generate_outreach.py",
        )
        gen.DATA_DIR = self.tmpdir / "data"
        gen.OUTPUTS_DIR = self.tmpdir / "outputs"
        sys.modules["generate_outreach"] = gen

        matrix_path = self.tmpdir / "data" / "test_client_accounts.json"
        ...
        # Override _matrix_path to point to tmpdir
        self.s2m._matrix_path_real = self.s2m._matrix_path if hasattr(self.s2m, "_matrix_path") else None
        ...
        import generate_outreach as _gen  # noqa: F401
        _gen.DATA_DIR = self.tmpdir / "data"
        _gen._matrix_path = _patched_path
        _gen.load_client_matrix = _patched_load
    ...
    def tearDown(self):
        shutil.rmtree(self.tmpdir, ignore_errors=True)
        for k in ["t_s2m", "generate_outreach"]:
...
class TestHubSpotCreateTask(unittest.TestCase):
    ...
    def tearDown(self):
        sys.modules.pop("t_hubspot", None)
